...
<body>
	...
	<div id="div">
		<p class="p">注册人员管理</p>
		<form
			action="delReg?offset=${offset}&pageNow=${pageNow}&signalSize=${signalSize}&type=${type}"
			method="post">
			<table class="table table1 " style="margin-top: 13px;">
				...
				<c:forEach var="var" varStatus="vs" items="${register}">
					<tr id="${var.id}" >
						...
						<select id="${var.id}" name="select" onchange="ModifyAuthority(this.value,this.id)" style="width: 100px;">
								<option selected="selected" value="0">注册人员</option>
								<option value="1">系统会员</option>
								<option value="2">系统管理员</option>
						</select>
						</td>
						<td style="text-align: center;"><a
							href="delReg?ids=${var.id}&offset=${offset}&pageNow=${pageNow}&signalSize=${signalSize}&type=0"
							onclick="checksig()">删除</a><a
							href="getInfoUser?id=${var.id}&offset=${offset}&pageNow=${pageNow}&signalSize=${signalSize}&type=0"
	...
	<div id="page">
		...
		<c:if test="${(pageCount>=1)}">
			<a href="register?offset=0&pageNow=1&type=0">首页</a>
		</c:if>
		<c:if test="${(pageCount>1) && (pageNow !=1) }">
			<a href="register?offset=${offset-10}&pageNow=${pageNow-1}&type=0">上一页</a>
		</c:if>
		<c:if
			test="${pageCount>1 && (pageNow !=pageCount)&&(pageNow!=pageCount) }">
			<a href="register?offset=${offset+10}&pageNow=${pageNow+1}&type=0">下一页</a>
		</c:if>
		<c:if test="${(pageCount!=0)}">
			<a
				href="register?offset=${(pageCount-1)*10}&pageNow=${pageCount}&type=0">尾页</a>
		</c:if>
