	...
	$(document).ready(function() {
		...
		$("#div .audio").each(function(index) {
			$(this).click(function() {
				...
				var Ahref = href + "&sortName=" + sortName;
				$(this).attr("href", Ahref);
				if (!confirm("您确定要发布它为" + sortName + "类型吗？")) {
					return false;
				}
			});
		});
...
<body>
	...
	<c:if test="${strutsPageBean2.getTotleRows()!=0}">
		...
		<form action="deleteVideo" method="POST">
			...
			<p>&nbsp;</p>
				<p>将视频发布到
					<select style="border-radius: 0px;" id="sortName" name="sortName">
						<c:forEach items="${sorts}" var="sorts">
							<option value="${sorts.getSortName()}">
								${sorts.getSortName()}</option>
						</c:forEach>
					</select><font color="red" size="2">亲：注意选择类型哦!</font>
				...
				<c:forEach items="${flashVedios}" var="flashVedios">
					<div class="div1">
						...
						<div class="div1_2">
							...
							<a title="发布" class="audio" name="audio"
								href="deleteVideo?currentPage=${strutsPageBean2.getCurrentPage()}&publish=publish&videoId=${flashVedios.getId() }"><img
								src="images/behind_img/Document-Check.png"></a>

							<c:if test="${strutsPageBean2.getTotleRows()%12 ==1}">
								<a title="删除"
									href="deleteVideo?currentPage=${strutsPageBean2.getCurrentPage()-1}&videoId=${flashVedios.getId() }&publish=delete"
									onclick="return deleteA()"><img
									src="images/behind_img/Garbage.png"></a>
							</c:if>
							<c:if test="${strutsPageBean2.getTotleRows()%12 !=1}">
								<a title="删除"
									href="deleteVideo?currentPage=${strutsPageBean2.getCurrentPage()}&videoId=${flashVedios.getId() }&publish=delete"
									onclick="return deleteA()"><img
									src="images/behind_img/Garbage.png"></a>
							</c:if>
						</div>
		...
		<div class="div1">
			...
			<div style="padding-left: 100px; width: 600px; float: left;">
				<input type="submit" id="publish"
					style="margin-left: 30px; background-color: #ff9600; width: 107px; height: 41px; color: #ffffff;"
					value="发布" /> <input type="submit" id="delete"
		...
		<div id="fenye1">
			<c:if test="${strutsPageBean2.getCurrentPage()>1}">
				<a href="vedio?currentPage=${strutsPageBean2.getPreviousPage()}"
					title="上一页">上一页</a>
			</c:if>
			...
			<c:if
				...
				<a href="vedio?currentPage=${strutsPageBean2.getNextPage()}"
					title="下一页">下一页</a>
			</c:if>
			<form action="vedio" method="post">
