...
<body>
	<c:if test="${strutsPageBean1.getTotleRows()!=0}">
		...
		<form action="deleteImages" method="post">
			...
			<p>&nbsp;</p>
				<p>将图片发布到
					<select id="sortName" style="border-radius: 0px; z-index: 100"
						name="sortName">
				...
				<c:forEach items="${images}" var="images">
					...
					<div class="div1">
						...
						<div class="div1_2">
							...
							<a class="audio"
								href="deleteImages?currentPage=${strutsPageBean1.getCurrentPage()}&publish=publish&imageId=${images.getId() }"
								name="publishImages"><img title="发布"
								src="images/behind_img/Document-Check.png"></a>
							<c:if test="${strutsPageBean1.getTotleRows()%12 ==1}">
								<a
									href="deleteImages?currentPage=${strutsPageBean1.getCurrentPage()-1}&publish=delete&imageId=${images.getId() }"
									onclick="return deleteA()" name="deleteImages"><img
									title="删除" src="images/behind_img/Garbage.png"></a>
							</c:if>
							<c:if test="${strutsPageBean1.getTotleRows()%12 !=1}">
								<a
									href="deleteImages?currentPage=${strutsPageBean1.getCurrentPage()}&imageId=${images.getId() }&publish=delete"
									onclick="return deleteA()" name="deleteImages"><img
									title="删除" src="images/behind_img/Garbage.png"></a>
							</c:if>
						</div>
		...
		<div id="fenye1">
			<c:if test="${strutsPageBean1.getCurrentPage()>1}">
				<a
					href="imagesTest?currentPage=${strutsPageBean1.getPreviousPage()}"
					title="上一页">上一页</a>
			</c:if>
			...
			<c:if
				...
				<a href="imagesTest?currentPage=${strutsPageBean1.getNextPage()}"
					title="下一页">下一页</a>
			</c:if>
			<form action="imagesTest" method="post">
