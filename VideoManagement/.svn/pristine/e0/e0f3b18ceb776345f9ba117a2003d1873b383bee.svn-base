...
<scriptsrc="http://html5media.googlecode.com/svn/trunk/src/html5media.min.js">
</script> <script type="text/javascript" src="js/base_Js/jquery-1.9.1.js"
...
var thunder_url = "http://localhost:8080/VideoManagement/flashResources/175929.avi";
...
<body>
		...
		<div style="float: left; width: 100%;margin-top: 10px;">
			<div style="width: 300px; float: left; margin-left: 25px;">
				<form style="width: 200px;"
					action="uploadVedio?currentPage=${strutsPageBean2.getPreviousPage()}&flashSortType=${flashSortType}"
					id="typeForm">
					<select name="flashSortType" id="flashSortType"
						onchange="changeType()">
						<c:choose>
							<c:when test="${flashSortType!=null}">
								<option value="${flashSortType }">${flashSortType }</option>
							</c:when>
							<c:otherwise>
								<option>选择分类</option>
							</c:otherwise>
	...
	<c:if test="${empty flashVedios}" >
		<div align="center" style="font-size: 14;color: #0088CC;">>>>>>>>暂无数据>>>>>></div>
	</c:if>
		<form
			action="delMoreFlash?currentPage=${strutsPageBean2.getCurrentPage()}&flashSortType=${flashSortType}"
			method="post" id="form">
			...
			<c:forEach items="${flashVedios}" var="flashVedios">
				<div class="div1">
					...
					<div class="div1_1">
						<a
							href="showFlashAction?path=${flashVedios.getUrl()}&imagePath=${flashVedios.getFlashImagePath()}"
							target="_blank"><img src="${flashVedios.getFlashImagePath()}"
							class="imgshow"></a>
					</div>
					<div class="div1_2">
						...
						<a
							href="delUploudFlash?id=${flashVedios.getId()}&currentPage=${strutsPageBean2.getCurrentPage()}&flashSortType=${flashSortType}"
	...
	<div id="fenye1">
		<c:if test="${strutsPageBean2.getCurrentPage()>1}">
			<a
				href="uploadVedio?currentPage=${strutsPageBean2.getPreviousPage()}&flashSortType=${flashSortType}"
				title="上一页">上一页</a>
		</c:if>
		...
		<c:if
			...
			<a
				href="uploadVedio?currentPage=${strutsPageBean2.getNextPage()}&flashSortType=${flashSortType}"
				title="下一页">下一页</a>
		</c:if>
		<form action="uploadVedio?flashSortType=${flashSortType}"
			method="post">
